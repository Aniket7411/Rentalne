  ...
  return (
    <div className="min-h-screen bg-slate-100 pb-24 sm:pb-16">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-4 sm:-mt-6 relative z-10">
        {/* Filter chips — English, flex-wrap like design */}
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          className="sticky top-0 z-30 -mx-4 px-4 py-4 sm:mx-0 sm:px-0 sm:static sm:py-0 bg-slate-100/95 sm:bg-transparent backdrop-blur-md sm:backdrop-blur-none border-b border-slate-200/90 sm:border-0"
        >
          <div className="rounded-xl bg-white p-4 shadow-md ring-1 ring-slate-200/80">
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-3">Filter by category</p>
              ...
              {SERVICE_TABS.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      inline-flex items-center gap-2 rounded-lg px-3.5 py-2.5 text-sm font-semibold transition-all
                      ${
                        isActive
                          ? 'bg-sky-500 text-white shadow-md shadow-sky-500/25'
                          : 'bg-slate-100 text-neutral-800 hover:bg-slate-200 ring-1 ring-slate-200/90'
                      }
                    `}
                  >
      ...
      <Link
        to="/contact"
        className="fixed bottom-5 right-5 z-40 inline-flex items-center gap-2 rounded-full bg-sky-500 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-sky-600/40 hover:bg-sky-600 transition-colors sm:bottom-8 sm:right-8"
      >
